...
Game.Map.prototype.isEmptyWater = function(x, y, z) {
    // Check if the tile is floor and also has no entity
    return this.getTile(x, y, z) == Game.Tile.waterTile &&
           !this.getEntityAt(x, y, z);
};
        ...
        (function() {
            ...
            map._fov.push(
                new ROT.FOV.DiscreteShadowcasting(function(x, y) {
                	var armor = Game.Screen.playScreen._player.getArmor();
                	if(armor && armor.describe() === 'xray goggles') {
                		return true;
                	} else {
...
Game.Map.prototype.getRandomWaterPosition = function(z) {
    // Randomly generate a tile which is a water tiles
    var x, y;
    do {
        x = Math.floor(Math.random() * this._width);
        y = Math.floor(Math.random() * this._height);
    } while(!this.isEmptyWater(x, y, z));
    return {x: x, y: y, z: z};
};

Game.Map.prototype.addEntityAtRandomPosition = function(entity, z) {
    var position = this.getRandomFloorPosition(z);
	if(entity.hasMixin('Swimmer')){
		position = this.getRandomWaterPosition(z);
	}
