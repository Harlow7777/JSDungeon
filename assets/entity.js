Game.Entity = function(properties) {
    ...
    this._color = properties['foreground'];
    this._map = null;
    this._alive = true;
    // Acting speed
...
Game.Entity.prototype.tryMove = function(x, y, z, map) {
    ...
    if (z < this.getZ()) {
        ...
        if (tile === Game.Tile.holeToCavernTile &&
        	...
        	Game.sendMessage(this, "You fell through a hole!");
    ...
    } else if ((tile.isWalkable() && !this.hasMixin('Swimmer')) || (tile === Game.Tile.waterTile && ((this.hasMixin(Game.EntityMixins.PlayerActor) && this.getArmor().describe() === 'snorkel') || this.hasMixin('Swimmer')))) {        
    ...
    // Check if the tile is diggable
        ...
        if (this && this.hasMixin(Game.EntityMixins.PlayerActor) && (this.getWeapon() && this.getWeapon().describe() === 'shovel')) {
            map.dig(x, y, z);
            return true;
        }
        // If not nothing we can do, but we can't 
