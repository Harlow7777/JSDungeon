...
Game.EntityMixins.GiantZombieActor = Game.extend(Game.EntityMixins.TaskActor, {
    init: function(template) {
        ...
        Game.EntityMixins.TaskActor.init.call(this, Game.extend(template, {
            'tasks' : ['rage', 'spawnSlime', 'hunt', 'wander']
        }));
        // We only want to grow the arm once.
        this._rageMode = false;
    },
    canDoTask: function(task) {
        // If we haven't raged and HP <= 20, then we can rage.
        if (task === 'rage') {
            return this.getHp() <= 20 && !this._rageMode;
    ...
    rage: function() {
        ...
        Game.sendMessageNearby(this.getMap(),
            ...
            'The zombie enters a frenzied rage!');
    },
    spawnSlime: function() {
...
Game.EntityMixins.Attacker = {
    ...
    attack: function(target) {
        ...
        if (target.hasMixin('Destructible')) {
            ...
            Game.sendMessage(this, '%d damage to the %s!', 
                [damage, target.getName()]);
            Game.sendMessage(target, '%s does %d damage!', 
                [this.getName(), damage]);
            ...
            if(!target.hasMixin(Game.EntityMixins.PlayerActor)){
        		...
        		Game.sendMessage(this, "%c{" + target._color + "}" + target._character + " " + hp + "/" + target.getMaxHp());
        	}
        }
    },
	listeners: {
...
Game.EntityMixins.ExperienceGainer = {
    ...
    init: function(template) {
        ...
        this._statOptions = [];
        this._statOptionsText = [];
        if (this.hasMixin('Attacker')) {
            this._statOptions.push(['Increase attack value', this.increaseAttackValue]);
            this._statOptionsText.push(['Increase attack value']);
        }
        if (this.hasMixin('Destructible')) {
            ...
            this._statOptionsText.push(['Increase defense value']);
            this._statOptionsText.push(['Increase max health']);
        }
        if (this.hasMixin('Sight')) {
            this._statOptions.push(['Increase sight range', this.increaseSightRadius]);
            this._statOptionsText.push(['Increase sight range']);
        }
    },
    getLevel: function() {
        return this._level;
    ...
    getStatOptionsText: function() {
    	return this._statOptionsText;
    },
